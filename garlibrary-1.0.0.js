...
(function (window, undefined) {
    ...
    gar.serialize = function (form) {
        ...
        eparts = [], field = null, i, len, j, optLen, option, optValue;
        for (i = 0, len = formEle.length; i < len; ++i) {
            ...
            switch (field.type) {
                ...
                case 'select-multiple':
                    if (field.name.length) {
                        for (j = 0, optLen = field.options.length; j < optLen; ++j) {
                            ...
                            if (option.selected) {
                                optValue = '';
                                if (option.hasAttribute) optValue = option.hasAttribute('value') ? option.value :
                                    option.text;
                ...
                default:
                    if (field.name.length) parts.push(encodeURIComponent(field.name) + '=' + encodeURIComponent(
                            field.value));
            }
        }
        return parts.join('&');
    ...
    gar.imitClick = function (ele) {
        ...
        event.initMouseEvent('click', true, true, doc.defaultView, 0, 0, 0, 0, 0, false, false, false, false, 0,
            null);
    ...
    gar.isType = function (obj, type) {
        return new RegExp(type.toLowerCase(), 'i')
    ...
    gar.each = function (obj, callback) {
        ...
        var val, i = -1,
            o, isArr = obj instanceof Array,
            l = obj.length;
        if (isArr)
            for (; o = obj[++i];) val = callback.call(o, i, o, l);
        else
            for (o in obj) val = callback.call(obj[o], ++i, obj[o], l, o);
        return obj;
    };
    ...
    gar.breakEach = function (arr, callback) {
        for (var i = -1, o; o = arr[++i];)
            if (callback(i, o) === false) break;
    };
    /**
     * decode queryString
     */
    gar.getQueryStringArgs = function () {
        ...
        for (var len = items.length; len--;) {
            ...
            if (decodeURIComponent(item[0])
                    .length) result[decodeURIComponent(item[0])] = decodeURIComponent(item[1]);
        }
        return result;
    };
    ...
    gar.getDocHeight = function () {
        ...
        return doc.compatMode == 'CSS1Compat' ? Math.max(doc.documentElement.scrollHeight, doc.documentElement.clientHeight) :
            Math.max(doc.body.scrollHeight, doc.body.clientHeight);
    };
    gar.getDocWidth = function () {
        return doc.compatMode == 'CSS1Compat' ? Math.max(doc.documentElement.scrollWidth, doc.documentElement.clientWidth) :
            Math.max(doc.body.scrollWidth, doc.body.clientWidth);
    };
    ...
    /**
     * get special parent node
     */
    gar.getParent = function (curNode, className) {
        var node = curNode;
        console.log(node, node.className, className, new RegExp(node.className)
            .test(className));
        while (!node.className || !new RegExp(node.className)
            .test(className)) {
    ...
    gar.addClass = function (eleOrObj, classname) {
        if (!eleOrObj.nodeType) {
            for (var className in eleOrObj)
                if (!new RegExp('\\b' + className + '\\b', 'gi')
                        .test(eleOrObj[className].className))
                    eleOrObj[className].className = eleOrObj[className].className.concat(' ' + className);
        } else if (!new RegExp('\\b' + classname + '\\b', 'gi')
                .test(eleOrObj.className))
    ...
    gar.removeClass = function (eleOrObj, classname) {
        if (!eleOrObj.nodeType) {
            for (var className in eleOrObj)
                if (new RegExp('\\b' + className + '\\b', 'gi')
                        .test(eleOrObj[className].className))
                    eleOrObj[className].className = ('' + eleOrObj[className].className + '')
                        .replace(className, '');
        } else if (new RegExp('\\b' + classname + '\\b', 'gi')
                .test(eleOrObj.className))
            eleOrObj.className = ('' + eleOrObj.className + '')
                .replace(classname, '');
    };
        ...
        setAll: function (name, subCookies, expires, path, domain, secure) {
            ...
            for (subName in subCookies)
                if (subName.length > 0 && subCookies.hasOwnProperty(subName))
                    subCookieParts.push(encodeURIComponent(subName) + '=' + encodeURIComponent(subCookies[
                            subName]));
    ...
    gar.getLocalStorage = function () {
        if (typeof localStorage == 'object') this.getLocalStorage = function () {
            ...
            return globalStorage[localhost.host];
        };
    ...
    gar.getFullPageWH = function (wOrh) {
        var pageWidth = doc.innerWidth,
            pageHeight = doc.innerHeight;
    ...
    gar.getEleSumH = function (ele) {
        return ele.offsetTop + ele.offsetHeight + (parseInt(gar.getStyle(ele, 'marginBottom')) ? parseInt(gar.getStyle(
                ele, 'marginBottom')) : 0)
    };
    /**
    ...
    if (!Function.fn.bind) {
        Function.fn.bind = function (obj) {
            var slice = [].slice,
                ...
                nop = function () {},
                bound = function () {
                    return self.apply(this instanceof nop ? this : (obj || {}), args.concat(slice.call(
                        arguments)));
                };
                    ...
                    if (e.touches.lengt === 1) {
                        switch (e.type) {
                            case 'touchstart':
                                ...
                                break;
                            case 'touchmove':
                                if (e.touches[0].clientY - _y > 50) return 'load';
                            case 'touchend':
    ...
    gar.formValidateStrategies = {
        ...
        limitedNum: function (val, len, errorMsg) {
            var min = len.split('-')[0],
                max = len.split('-')[1],
                val = val[0] === '0' ? val.substring(1) : val;
    ...
    gar.Dictionary.prototype = {
        ...
        showAll: function (fn) {
            for (var key in Object.keys(this.dataStore)
                .sort()) fn(key, this.dataStore[key]);
        },
        count: function () {
            var n = 0;
    ...
    gar.heapSort = function (array) {
        var heapify = function (arr, x, len) {
            if (Object.prototype.toString.call(arr)
                    .slice(8, -1) === 'Array' && typeof x === 'number') {
                var l = 2 * x + 1,
                    r = 2 * x + 2,
                    largest = x,
                    temp;
